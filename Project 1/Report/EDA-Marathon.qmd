...
# Table 1
round(table(data$Flag)/sum(table(data$Flag)) * 100,2) %>%
  kable(col.names = c('Flag', 'Percentage (%)')) %>%
  kable_styling()
```
...
```{r}
mod <- glm(Time ~ Age..yr.+ I(Age..yr.^2)+as.factor(Sex), family = gaussian(), data = data_record)
summary(mod)
plot(data_record$Age..yr., predict(mod))
```
...
```{r}
plot(data_record_weight$X.CR ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$Td..C ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$Tw..C ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$Tg..C ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$WBGT ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$SR.W.m2 ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$DP ,data_record_weight$Avg_Cr_Diff)
plot(data_record_weight$Wind ,data_record_weight$Avg_Cr_Diff)
...
mod <- step(mod, scope = list(lower = low_scope, upper = high_scope), direction = 'both')
